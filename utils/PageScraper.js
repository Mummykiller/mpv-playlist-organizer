...
window.MPV.PageScraper = class PageScraper {
    constructor() {
    ...
    scrapePageDetails(url) {
        ...
        // Priority 1: URL Hash (common for anime sites)
        ...
        // Priority 2: Active/Selected episode elements
        if (!episode) {
        ...
        // Broader search if still missing
        if (!season || !episode) {
        ...
        // Surgically remove matched season/ep text
        ...
        return { 
            url: window.MPV.sanitizeString(url), 
            title: window.MPV.sanitizeString(resultTitle) 
        };
    }
};
