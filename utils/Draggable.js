...
window.MPV.Draggable = class Draggable {
    constructor(element, handle, options = {}) {
        this.element = element;
        this.handle = handle;
        this.options = { dragButton: 0, onDragStart: () => {}, onDragMove: () => {}, onDragEnd: () => {}, clamp: true, ...options };
    ...
    onMouseDown(e) {
        if (e.button !== this.options.dragButton || this.options.onDragStart(e) === false) return;
        e.preventDefault();
    ...
    onMouseUp(e) {
        ...
        let pos;
        if (elCenter < center) pos = { left: `${(rect.left / vw) * 100}%`, top: `${(rect.top / vh) * 100}%`, right: 'auto', bottom: 'auto' };
        else pos = { left: 'auto', top: `${(rect.top / vh) * 100}%`, right: `${((vw - rect.right) / vw) * 100}%`, bottom: 'auto' };
        this.options.onDragEnd(e, pos);
    }

    onContextMenu(e) { if (this.options.dragButton === 2) e.preventDefault(); }
    attach() {
