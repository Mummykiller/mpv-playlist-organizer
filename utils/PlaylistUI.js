...
window.MPV.PlaylistUI = class PlaylistUI {
        ...
        if (playlist && playlist.length > 0) {
            ...
            playlist.forEach((item, index) => {
                ...
                if (highlightEnabled && lastPlayedId && item.id === lastPlayedId) {
                    itemDiv.classList.add(isFolderActive ? 'active-item' : 'last-played-item');
                }
    ...
    _formatTitle(urlSpan, item) {
        ...
        if (titleParts.length > 1 && (/^(s\d+)?e\d+(\.\d+)?$/i.test(titleParts[0].trim()) || isYT)) {
