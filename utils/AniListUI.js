...
(() => {
	...
	MPV.AniListUI = class AniListUI {
		...
		bindEvents() {
			...
			this.shadowRoot
				.getElementById("btn-close-anilist-panel")
				?.addEventListener("click", () => this.toggleVisibility(false));
			this.shadowRoot
				.getElementById("btn-refresh-anilist")
				?.addEventListener("click", () =>
					this.fetchReleases(true, this.currentOffset),
				);
			...
			this.shadowRoot
				.getElementById("btn-pin-anilist-panel")
			...
			this.controllerShadowRoot
				.getElementById("btn-toggle-anilist-left")
				?.addEventListener("click", toggleHandler);
			this.controllerShadowRoot
				.getElementById("btn-toggle-anilist-right")
				?.addEventListener("click", toggleHandler);
			const dragHandle = this.shadowRoot.querySelector(".anilist-panel-header");
			const resizeHandle = this.shadowRoot.getElementById(
				"anilist-resize-handle",
			);
			if (this.panelHost && dragHandle) {
				new window.MPV_INTERNAL.Draggable(this.panelHost, dragHandle, {
					dragButton: 0,
			...
			if (this.panelHost && resizeHandle) {
				new window.MPV_INTERNAL.Resizable(this.panelHost, resizeHandle, {
					minWidth: 250,
					minHeight: 200,
					onResizeEnd: (size) =>
						this.controller.savePreference({ anilistPanelSize: size }),
				});
			}
		}
		...
		toggleVisibility(forceState, savePref = true) {
			...
			const isControllerMinimized =
				this.uiManager.controllerHost.style.display === "none";
			if (
				shouldBeVisible &&
				(!this.isEnabled || (this.forceAttached && isControllerMinimized))
			)
				shouldBeVisible = false;
			const leftBtn = this.controllerShadowRoot.getElementById(
				"btn-toggle-anilist-left",
			);
			const rightBtn = this.controllerShadowRoot.getElementById(
		...
		snapToController() {
			...
			if (
				!this.panelHost ||
				!this.uiManager.controllerHost ||
				this.panelHost.style.display === "none"
			)
				return;
			const controllerRect =
				this.uiManager.controllerHost.getBoundingClientRect();
			const panelWidth = this.panelHost.offsetWidth;
			const gap = 10;
			const controllerCenter = controllerRect.left + controllerRect.width / 2;
			const screenCenter = window.innerWidth / 2;
			const newLeft =
				controllerCenter < screenCenter
					? controllerRect.right + gap
					: controllerRect.left - panelWidth - gap;
		...
		async fetchReleases(forceRefresh = false, daysOffset = 0) {
			...
			if (btnToday) {
				...
				if (daysOffset !== 0) {
					...
					btnToday.textContent = targetDate.toLocaleDateString(undefined, {
						month: "short",
						day: "numeric",
					});
				} else {
			...
			try {
				const releases = await MPV.AniListRenderer.fetchReleases(
					forceRefresh,
					daysOffset,
				);
				if (this.controller.checkContext())
					MPV.AniListRenderer.render(container, releases, daysOffset);
			} catch (error) {
