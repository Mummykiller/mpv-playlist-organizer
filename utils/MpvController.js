	...
	MPV.MpvController = class MpvController {
		constructor() {
			...
			this.nav = new MPV.NavigationObserver({
				...
				onValidate: () => {
					if (
						!document.getElementById("m3u8-controller-host") &&
						!this.isTearingDown
					) {
						this.teardown();
		...
		async init() {
			...
			this.messageListener = (req, sender, send) =>
				this.handleMessage(req, sender, send);
		...
		handleMessage(request, sender, sendResponse) {
			...
			const action =
				request.action || (request.foldersChanged ? "foldersChanged" : null);
		...
		_syncUiToState(state) {
			...
			if (this.anilistUI) {
				if (
					this.anilistUI.panelHost.style.display === "none" &&
					state.anilistVisible
				) {
				...
				if (this.anilistUI.panelHost) {
					...
					this.anilistUI.panelHost.style.setProperty(
						"--anilist-item-width",
						`${effectiveWidth}px`,
					);
					this.anilistUI.panelHost.style.setProperty(
			...
			const compactUi = this.ui.shadowRoot.getElementById(
			...
			if (fullUi && compactUi && modeBtn) {
				...
				modeBtn.querySelector(".icon-full-ui").style.display = isFull
					? "block"
					: "none";
				modeBtn.querySelector(".icon-compact-ui").style.display = isFull
					? "none"
					: "block";
			}
		...
		async applyInitialState(positionOverride = null) {
			try {
				...
				if (position && this.ui.controllerHost)
					Object.assign(this.ui.controllerHost.style, position);

				if (this.anilistUI) {
					if (prefs.anilistPanelPosition) {
						Object.assign(
							this.anilistUI.panelHost.style,
							prefs.anilistPanelPosition,
						);
						this.anilistUI.isManuallyPositioned = true;
					}
					if (prefs.anilistPanelSize)
						Object.assign(
							this.anilistUI.panelHost.style,
							prefs.anilistPanelSize,
						);
				...
				if (this.ui.minimizedHost && prefs.minimizedStubPosition) {
					Object.assign(
						this.ui.minimizedHost.style,
						prefs.minimizedStubPosition,
					);
					this.ui.minimizedHost.classList.remove("top-left", "top-right");
				}

				const shouldBeMinimized =
					typeof prefs.minimized === "boolean"
						? prefs.minimized
						: prefs.mode === "minimized";
		...
		_syncFolderChange(req) {
			...
			if (full && full.value !== req.folderId) {
				...
				const compact = this.ui.shadowRoot.getElementById(
		...
		_handleRightClickScrape(send) {
			...
			if (
				...
				this.lastRightClickedElement
			) {
				const videoContainer = this.lastRightClickedElement.closest(
					"ytd-rich-item-renderer, ytd-video-renderer, ytd-grid-video-renderer, ytd-compact-video-renderer",
				);
				if (videoContainer) {
					const linkEl = videoContainer.querySelector(
						"a#video-title-link, a#thumbnail, a.yt-simple-endpoint",
					);
					...
					const channelSelectors = [
						...
						"#byline-container .yt-formatted-string",
						"#owner-name a",
					];
			...
			if (!title || !url) {
				const details = this.pageScraper.scrapePageDetails(
		...
		showPageLevelConfirmation(message) {
			return new Promise((resolve) => {
				...
				shadowRoot.getElementById("page-level-modal-message").textContent =
					message;
				...
				shadowRoot.getElementById("page-level-modal-confirm-btn").onclick =
					() => close(true);
				shadowRoot.getElementById("page-level-modal-cancel-btn").onclick = () =>
					close(false);
		...
		async updateFolderDropdowns(targetId, targetLastPlayed, targetIsActive, targetIsPaused, targetNeedsAppend) {
			...
			const compactSelect = this.ui.shadowRoot?.getElementById(
				"compact-folder-select",
			);
		...
		_bindControllerEvents() {
			...
			// --- Core UI Toggles ---
			root
				.getElementById("btn-toggle-minimize")
				?.addEventListener("click", () => this.setMinimizedState(true));
			root
				.getElementById("btn-toggle-pin")
			...
			root
				...
				?.addEventListener("click", () => {
					const newMode =
						this.state.state.uiMode === "full" ? "compact" : "full";
			...
			root.getElementById("btn-add")?.addEventListener("click", async () => {
				...
				if (fid && this.state.state.detectedUrl) {
					const result = await this.bridge.send("add", fid, {
						data: this.pageScraper.scrapePageDetails(
			...
			root
				.getElementById("btn-compact-add")
				?.addEventListener("click", async () => {
					const fid = root.getElementById("compact-folder-select")?.value;
					if (fid && this.state.state.detectedUrl) {
						const result = await this.bridge.send("add", fid, {
			...
			root
				.getElementById("btn-compact-clear")
				?.addEventListener("click", async () => {
					const fid = root.getElementById("compact-folder-select")?.value;
					if (!fid) return;
					const confirmed =
			...
			root
				.getElementById("btn-compact-close-mpv")
				?.addEventListener("click", async () => {
					const fid = root.getElementById("compact-folder-select")?.value;
					if (!fid) return;
			...
			root
				...
				?.addEventListener("click", () =>
					this.state.update({ logVisible: !this.state.state.logVisible }),
				);
			...
			root.getElementById("btn-filter-info")?.addEventListener("click", () => {
				const filters = {
					...this.state.state.logFilters,
					info: !this.state.state.logFilters.info,
				};
			...
			root.getElementById("btn-filter-error")?.addEventListener("click", () => {
				const filters = {
					...this.state.state.logFilters,
					error: !this.state.state.logFilters.error,
				};
				this.state.update({ logFilters: filters });
			});
			...
			minRoot
				...
				?.addEventListener("click", async (e) => {
					...
					if (isStreamPresent && !isAlreadyInPlaylist && e.button === 0) {
						...
						const urlToUse =
							this.state.state.detectedUrl ||
							(isYouTube ? window.location.href : null);
						...
						if (targetFolderId) {
							...
							this.addLogEntry({
								text: `[Content]: Adding detected stream via minimized stub...`,
								type: "info",
							});
						...
						console.log(
							"[Stub Click] Logic falling through to setMinimizedState(false)",
						);
						this.setMinimizedState(false);
					}
				});
			minRoot
				?.getElementById("m3u8-minimized-play-btn")
				?.addEventListener("click", () => {
					const fid = getFolderId();
					if (fid) this.sendCommandToBackground("play", fid);
				});
		...
		updateAddButtonState() {
			...
			const compactAddBtn =
				this.ui.shadowRoot.getElementById("btn-compact-add");
			// Ensure we are looking inside the shadowRoot of the minimizedHost
			const minimizedStub = this.ui.minimizedHost?.shadowRoot?.getElementById(
				"m3u8-minimized-stub",
			);
			...
			if (!url) {
				...
				this.updateStatusBanner(
					isYouTube ? "YouTube Video detected" : "Stream/video detected",
					true,
				);
		...
		_handleResize() {
			...
			if (isOffScreen) {
				...
				if (originalLeft <= maxX && originalTop <= maxY)
					this.preResizePosition = null;
			}
		}
