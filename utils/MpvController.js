 ...
 * Orchestrates Logic modules and UI components.
 */
window.MPV = window.MPV || {};

window.MPV.MpvController = class MpvController {
    constructor() {
        this.isTearingDown = false;
        this.tabId = null;
        this.preFullscreenPosition = null;
        this.preResizePosition = null; // New: Resize tracking
        this.lastRightClickedElement = null;
        this._lastUpdateHash = ''; // New: Smart update tracking

        // 1. Initialize Logic Modules (Namespaced)
        this.bridge = new window.MPV.MessageBridge({
            onLog: (log) => this.addLogEntry(log)
        });

        this.state = new window.MPV.ContentState({
            onUpdate: (state) => this._syncUiToState(state)
        });

        this.nav = new window.MPV.NavigationObserver({
            onNavigation: (url) => this._handleNavigation(url),
            onMutation: () => this._handleDomMutation(),
            onValidate: () => {
                // Point 1: SPA Recovery - If host is gone, trigger recovery
                if (!document.getElementById('m3u8-controller-host') && !this.isTearingDown) {
                    this.teardown();
                    setTimeout(() => window.mpvStartInitialization?.(), 100);
                }
            }
        });

        // 2. Initialize UI Managers
        this.ui = new window.MPV.UIManager();
        this.playlistUI = null;
        this.anilistUI = null;
        this.pageScraper = new window.MPV.PageScraper();
    ...
    /**
     * Entry Point: Sets up the controller and its modules.
     */
    async init() {
        ...
        this.playlistUI = new window.MPV.PlaylistUI(this, this.ui);
        this.anilistUI = new window.MPV.AniListUI(this, this.ui);
    ...
    _syncUiToState(state) {
        ...
        // Point 3: Smart Update Hashing (Performance)
        const relevantState = {
        ...
        if (this.anilistUI) {
             // Sync Visibility
             // We pass false to savePref to avoid recursion or redundant saves
    ...
    async applyInitialState(positionOverride = null) {
        try {
            ...
            // Update scraper filter words if present
    ...
    _handleDomMutation() {
        ...
        if (!this.state.state.detectedUrl) {
            this.checkForYouTubeURL();
            this.updateAddButtonState();
        }
    }
    ...
    _handleRightClickScrape(send) {
        ...
        // Specialized YouTube logic for right-clicking thumbnails
        ...
        // Generic link handling for other sites if title still null
        ...
        // Fallback to PageScraper if still missing
        if (!title || !url) {
    ...
    async updateFolderDropdowns(targetId, targetLastPlayed, targetIsActive) {
        ...
        // Point 6: Atomic Dropdown Updates
    ...
    /**
     * Toggles the loading state on all play buttons.
     */
    ...
    /**
     * Toggles the active (playing) state on all play buttons.
     */
    ...
    checkForYouTubeURL() {
        ...
        const isWatchPage = currentUrl.includes('youtube.com/watch') || currentUrl.includes('youtu.be/');
        const isPlaylistPage = currentUrl.includes('youtube.com/playlist');

        if (isWatchPage || isPlaylistPage) {
            let url = currentUrl;
            if (isWatchPage && typeof window.MPV.normalizeYouTubeUrl === 'function') {
                url = window.MPV.normalizeYouTubeUrl(currentUrl);
            }
            this.state.update({ detectedUrl: window.MPV.sanitizeString(url) });
        }
    }

    _startHeartbeat() {
        this.heartbeatInterval = setInterval(this._safeWrap(async () => {
            // Send a lightweight ping/heartbeat to the background script
            // to ensure the connection is still alive.
            try {
                await this.bridge.send('heartbeat');
            } catch (e) {
                // If the message fails, it usually means the context is invalidated.
    ...
    _bindControllerEvents() {
        ...
        this.resizeListener = window.MPV.debounce(() => this._handleResize(), 250);
    ...
    updateAddButtonState() {
        ...
        const isPlaylist = url && /^https?:\/\/((www|music)\.)?youtube\.com\/playlist\?list=/.test(url);
    ...
    handleGlobalKeydown(e) {
        ...
        if (state.kb_add_playlist && combo === normalize(state.kb_add_playlist)) {
            ...
            e.preventDefault(); e.stopPropagation(); this.bridge.send('switch_playlist');
        } else if (state.kb_open_popup && combo === normalize(state.kb_open_popup)) {
            e.preventDefault(); e.stopPropagation(); this.bridge.send('open_popup');
        }
    }
    ...
    normalizeKeyCombo(e) {
        ...
        if (key === 'control') return null; // Don't trigger on modifier alone
    ...
    handleFullscreenChange() {
        ...
        if (document.fullscreenElement) {
            ...
            // Point 5: Instant Fullscreen Restore
