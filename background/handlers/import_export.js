...
export async function handleImportFromFile(request, sender) {
	...
	try {
		const baseFolderName = sanitizeString(
				...
				.map((item) => {
					if (typeof item === "string") {
						...
						const newItem = {
							...
							title: options.preserveTitle
								? sanitizeString(item.title || item.url)
		...
		if (
			importedData &&
			importedData.type === "mpv_playlist_organizer_settings"
		) {
		...
		if (Array.isArray(importedData)) {
			...
			foldersToImport[baseFolderName] = {
				playlist: processPlaylist(importedData),
			};
			...
			for (const key in importedData) {
				...
				if (folderContent && Array.isArray(folderContent.playlist)) {
					...
					if (options.preserveLastPlayed && folderContent.last_played_id) {
						if (
							folder.playlist.some((i) => i.id === folderContent.last_played_id)
						) {
		...
		if (Object.keys(foldersToImport).length === 0)
			return {
				success: true,
				message: `Import file was empty or incompatible.`,
			};
...
async function handleImportSettings(importedData, filename) {
	try {
		...
		const localDependencyStatus =
			localData.settings.ui_preferences?.global?.dependencyStatus;
...
export async function handleExportSettings(request) {
	...
	if (
		filteredSettings.ui_preferences &&
		filteredSettings.ui_preferences.global
	) {
...
export async function handleExportFolderPlaylist(request) {
	if (!request.filename || !request.folderId)
		return { success: false, error: "Missing filename or folderId." };
	const data = await storage.get();
	...
	if (!folder || !folder.playlist)
		return { success: false, error: `Folder empty.` };
